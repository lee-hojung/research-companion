...
Design note on query construction:
  - Retrieval query uses policy + population only (NOT your findings).
    ...
    not papers with similar results — which would introduce circularity.
...
def build_or_update_index(notes_folder, index_file):
    ...
    print(f"✅ Index ready: {len(index)} notes ({updated} updated)\n")
    return index
...
1. Write the citation as: **Author (Year)** — *Title*
...
- Be concise — output will be used directly in academic writing
...
def main():
    print('=' * 60)
    print('📚 Literature Review Assistant')
    print('=' * 60)
    ...
    if not index:
        print('❌ No notes found. Run step1_analysis.py first to generate notes.')
        return
    ...
    my_population = input('\nTarget population (optional — press Enter to skip):\n> ').strip()
    my_findings = input('\nYour key findings:\n> ').strip()

    if not my_policy or not my_findings:
        print('❌ Policy and findings are required.')
        return

    # Retrieval query: policy + population only (no findings — see module docstring)
